...
def initialize_roles(subsession: Subsession, allocation_method) -> None:
    ...
    allowance_allocation = calculate_optimal_allowance_allocation(
        players, subsession.market_price, subsession.carbon_multiplier, allocation_method
    )
    ...
    if config.carbon_trading_show_detailed_calculation:
        ...
        if output_format == "detailed":
            ...
            print(
                f"Parameters: p = {allowance_allocation['config']['market_price']}, "
            ...
            print(
                f"碳權分配完成：總排放={allowance_allocation['TE_opt_total']:.{decimal_places}f}, "
    ...
    for i, p in enumerate(players):
        print(
            f"玩家 {i + 1}: {'Dominant' if p.is_dominant else 'Non-dominant'}, "
...
def calculate_optimal_allowance_allocation(
    ...
    def _allocate_discrete_share(indices: list[int], total: int) -> dict[int, int]:
        ...
        base = total // n
        remainder = total % n
        allocations = dict.fromkeys(indices, base)
        if remainder > 0:
...
class TradingMarket(Page):
    ...
    def vars_for_template(player):
        ...
        profit_table = [
            {"quantity": int(qi), "marginal_cost": round(mc, 2), "profit": round(float(p), 2)}
            for qi, mc, p in zip(q, marginal_costs, profit, strict=False)
        ]
    ...
    def live_method(player: Player, data: dict[str, Any]) -> dict[int, dict[str, Any]]:
        ...
        if data.get("type") == "submit_offer":
            ...
            print(
                f"玩家 {player.id_in_group} 提交{direction}單: "
        ...
        # 處理接受訂單
            ...
            print(f"玩家 {player.id_in_group} 接受{offer_type}單: 對象玩家={target_id}, 價格={price}, 數量={quantity}")
            ...
            if result.get("notifications"):
                ...
                for p in group.get_players():
                    state = TradingMarket.market_state(p)
                    if p.id_in_group in result["notifications"]:
                        state["notification"] = {"type": "success", "message": result["notifications"][p.id_in_group]}
                    market_states[p.id_in_group] = state
                return market_states
        ...
        # 處理取消訂單
            ...
            print(f"玩家 {player.id_in_group} 取消{direction}單: 價格={price}, 數量={quantity}")
    ...
    def market_state(player: Player) -> dict[str, Any]:
        ...
        try:
            # 提取玩家自己的買單和賣單
            my_buy_offers = [
                {"player_id": int(pid), "price": int(float(price)), "quantity": int(qt)}
                for pid, price, qt in buy_sorted
                if int(pid) == player.id_in_group
            ]
            my_sell_offers = [
                {"player_id": int(pid), "price": int(float(price)), "quantity": int(qt)}
...
class ProductionDecision(Page):
    ...
    def vars_for_template(player):
        ...
        # 添加防護措施，確保不會除以零
        max(0.001, float(player.marginal_cost_coefficient))

        # 添加防護措施，確保current_cash為正數
        max(0, float(player.current_cash))
...
class Results(Page):
    ...
    def vars_for_template(player: Player):
        ...
        current_profit = current_total_final_value - (
            player.initial_capital if hasattr(player, "initial_capital") else C.INITIAL_CAPITAL
        )
        ...
        return {
            ...
            "reset_cash": C.RESET_CASH_EACH_ROUND,
            # 碳權信息
            "permits": player.field_maybe_none("permits")
            or player.current_permits,  # 初始分配的碳權（如果為None則使用當前碳權）
