...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("database_cleaning.log", encoding="utf-8"), logging.StreamHandler()],
)
...
class DatabaseCleaner:
    ...
    def __init__(self, db_path: str = "db.sqlite3"):
        ...
        self.stage_tables = {
            ...
            "muda": {"player": "stage_muda_player", "group": "stage_muda_group", "subsession": "stage_muda_subsession"},
            "carbon_trading": {
    ...
    def clean_control_group_data(self):
        ...
        # 1. 檢查生產量約束
        invalid_production = df[(df["production"] < 0) | (df["production"] > df["max_production"])]
        ...
        if not negative_cost.empty:
            ...
            self.cleaning_report["issues_found"].append(
                {"table": table_name, "issue": "總成本為負值", "count": len(negative_cost)}
            )
            # 將負成本設為0
        ...
        if issues_found > 0:
            ...
            self.cleaning_report["corrections_made"].append(
                {"table": table_name, "corrections": issues_found, "description": "對照組數據清理完成"}
            )
    ...
    def clean_carbon_tax_data(self):
        ...
        if "carbon_tax_paid" in df.columns:
            ...
            if self.validate_table_exists(subsession_table):
                ...
                if not tax_mismatch.empty:
                    ...
                    self.cleaning_report["issues_found"].append(
                        {"table": table_name, "issue": "碳稅計算不一致", "count": len(tax_mismatch)}
                    )
                    # 修正碳稅
        ...
        if "tax_rate" in df.columns:
            ...
            if not invalid_tax_rate.empty:
                ...
                self.cleaning_report["issues_found"].append(
                    {"table": table_name, "issue": "稅率超出合理範圍", "count": len(invalid_tax_rate)}
                )
        ...
        if all(col in df.columns for col in ["revenue", "total_cost", "carbon_tax_paid", "net_profit"]):
            ...
            if not profit_mismatch.empty:
                ...
                self.cleaning_report["issues_found"].append(
                    {"table": table_name, "issue": "稅後利潤計算不一致", "count": len(profit_mismatch)}
                )
        ...
        if issues_found > 0:
            ...
            self.conn.commit()
            self.cleaning_report["corrections_made"].append(
                {"table": table_name, "corrections": issues_found, "description": "碳稅組數據清理完成"}
            )
    ...
    def clean_muda_data(self):
        ...
        if all(col in df.columns for col in ["cash", "current_cash"]):
            ...
            if not cash_mismatch.empty:
                ...
                self.cleaning_report["warnings"].append(
                    {"table": table_name, "warning": "現金餘額字段不一致", "count": len(cash_mismatch)}
                )
            ...
            for idx, row in df.iterrows():
                if pd.notna(row["submitted_offers"]):
                    try:
                        ...
                        df.at[idx, "submitted_offers"] = "[]"
                        self.cleaning_report["issues_found"].append(
                            {"table": table_name, "issue": "JSON格式錯誤", "record_id": row.get("id", idx)}
                        )
        ...
        if all(col in df.columns for col in ["buy_quantity", "buy_price", "sell_quantity", "sell_price"]):
            # 檢查負值
            negative_values = df[
                (df["buy_quantity"] < 0) | (df["sell_quantity"] < 0) | (df["buy_price"] < 0) | (df["sell_price"] < 0)
            ]
            if not negative_values.empty:
                issues_found += len(negative_values)
                self.cleaning_report["issues_found"].append(
                    {"table": table_name, "issue": "交易數據包含負值", "count": len(negative_values)}
                )
                # 將負值設為0
        ...
        if all(col in df.columns for col in ["current_cash", "current_items", "personal_item_value", "total_value"]):
            ...
            if not value_mismatch.empty:
                ...
                self.cleaning_report["issues_found"].append(
                    {"table": table_name, "issue": "總資產價值計算不一致", "count": len(value_mismatch)}
                )
        ...
        if issues_found > 0:
            ...
            self.conn.commit()
            self.cleaning_report["corrections_made"].append(
                {"table": table_name, "corrections": issues_found, "description": "MUDA組數據清理完成"}
            )
    ...
    def clean_carbon_trading_data(self):
            ...
            for _, group_row in group_df.iterrows():
                ...
                if pd.notna(group_row["trade_history"]):
                    try:
                        ...
                        issues_found += 1
                        self.cleaning_report["issues_found"].append(
                            {"table": group_table, "issue": "交易歷史JSON格式錯誤", "group_id": group_row.get("id")}
                        )
        ...
        if all(col in df.columns for col in ["total_bought", "total_sold", "total_spent", "total_earned"]):
            # 檢查負值
            negative_trading = df[
                (df["total_bought"] < 0) | (df["total_sold"] < 0) | (df["total_spent"] < 0) | (df["total_earned"] < 0)
            ]
            if not negative_trading.empty:
                issues_found += len(negative_trading)
                self.cleaning_report["issues_found"].append(
                    {"table": table_name, "issue": "交易統計包含負值", "count": len(negative_trading)}
                )
                # 修正負值
        ...
        if all(col in df.columns for col in ["current_cash", "revenue", "total_cost", "initial_capital"]):
            ...
            if "net_profit" in df.columns:
                ...
                if not profit_mismatch.empty:
                    issues_found += len(profit_mismatch)
                    self.cleaning_report["issues_found"].append(
                        {"table": table_name, "issue": "最終利潤計算不一致", "count": len(profit_mismatch)}
                    )
                    df["net_profit"] = calculated_profit
        ...
        if issues_found > 0:
            ...
            self.conn.commit()
            self.cleaning_report["corrections_made"].append(
                {"table": table_name, "corrections": issues_found, "description": "碳交易組數據清理完成"}
            )
    ...
    def export_cleaned_data(self, output_dir: str = "cleaned_data"):
        ...
        for stage, tables in self.stage_tables.items():
            ...
            for _table_type, table_name in tables.items():
                if not self.validate_table_exists(table_name):
                    continue
    ...
    def run_full_cleaning(self, export_data: bool = True, generate_report: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            total_issues = sum(
                correction.get("corrections", 0) for correction in self.cleaning_report["corrections_made"]
            )
...
def main():
    ...
    result = cleaner.run_full_cleaning(export_data=not args.no_export, generate_report=not args.no_report)
