...
def update_price_history(subsession: BaseSubsession, trade_price: float, event: str = "trade") -> list[dict[str, Any]]:
    """
    更新價格歷史記錄
...
def record_trade(group: BaseGroup, buyer_id: int, seller_id: int, price: float, quantity: int) -> list[dict[str, Any]]:
    """
    記錄交易
...
def check_duplicate_order(orders: list[list], price: int, quantity: int) -> bool:
    """
    ...
    """
    for order in orders:
        if float(order[1]) == price and int(order[2]) == quantity:
            return True
    return False


def validate_order(player: BasePlayer, direction: str, price: int, quantity: int, item_name: str = "物品") -> None:
    """
    驗證訂單有效性
...
def find_matching_orders(
    orders: list[list], player_id: int, price: float, quantity: int, is_buy_order: bool
) -> list[tuple[int, list]]:
    """
    尋找匹配的訂單
...
def execute_trade(
    ...
    print(f"成功交易: 買方{buyer.id_in_group} <- 賣方{seller.id_in_group}, 價格{price}, 數量{quantity}")
...
def process_new_order(
    ...
    try:
        ...
        return {"type": "fail", "notifications": {player.id_in_group: str(e)}}
    ...
    if direction == "buy":
        if check_duplicate_order(buy_orders, price, quantity):
            return {
                "type": "fail",
                "notifications": {player.id_in_group: f"市場上已有價格 {price} 且數量 {quantity} 的買單！"},
            }
    else:  # sell
        if check_duplicate_order(sell_orders, price, quantity):
            return {
                "type": "fail",
                "notifications": {player.id_in_group: f"市場上已有價格 {price} 且數量 {quantity} 的賣單！"},
            }
    ...
    # 尋找匹配的訂單
    if direction == "buy":
        matching_orders = find_matching_orders(sell_orders, player.id_in_group, price, quantity, True)
        ...
        if matching_orders:
            ...
            try:
                ...
                sell_orders = [
                    ...
                    if not (
                        int(o[0]) == seller_id
                        and float(o[1]) == float(best_order[1])
                        and int(o[2]) == int(best_order[2])
                    )
                ]
        ...
        matching_orders = find_matching_orders(buy_orders, player.id_in_group, price, quantity, False)
...
def process_accept_offer(
    ...
    if target_id == player.id_in_group:
        return {"type": "fail", "notifications": {player.id_in_group: "不能接受自己的訂單"}}
    ...
    try:
        if offer_type == "sell":
            ...
            sell_orders = [
                o
                for o in sell_orders
                if not (int(o[0]) == target_id and float(o[1]) == price and int(o[2]) == quantity)
            ]
            save_orders(group, buy_orders, sell_orders)
            ...
            if current_items < quantity:
                return {"type": "fail", "notifications": {player.id_in_group: f"您的{item_name}不足"}}
            ...
            buy_orders, sell_orders = parse_orders(group)
            buy_orders = [
                o for o in buy_orders if not (int(o[0]) == target_id and float(o[1]) == price and int(o[2]) == quantity)
            ]
            save_orders(group, buy_orders, sell_orders)
        ...
        print(f"接受訂單失敗: {e}")
        return {"type": "fail", "notifications": {player.id_in_group: "交易失敗：找不到交易對象"}}


def calculate_locked_resources(
    player: BasePlayer, buy_orders: list[list], sell_orders: list[list]
) -> tuple[float, int]:
    """
    計算已鎖定的資源
    ...
    # 計算鎖定的物品（賣單）
    locked_items = sum(int(order[2]) for order in sell_orders if int(order[0]) == player_id)
...
def filter_top_orders_for_display(orders: list[list], max_per_quantity: int = 3) -> list[list]:
    ...
    # 對每個數量組排序並取前N筆
    filtered_orders = []
    for _quantity, group in quantity_groups.items():
...
def record_submitted_offer(player, direction, price, quantity):
    ...
    submitted_offers.append({"timestamp": timestamp, "direction": direction, "price": price, "quantity": quantity})
...
def cancel_specific_order(group, player_id, direction, price, quantity):
    ...
    if direction == "buy":
        buy_orders = [
            o for o in buy_orders if not (int(o[0]) == player_id and float(o[1]) == price and int(o[2]) == quantity)
        ]
    else:
        sell_orders = [
            o for o in sell_orders if not (int(o[0]) == player_id and float(o[1]) == price and int(o[2]) == quantity)
        ]
