...
from configs.config import ConfigConstants, config
...
def _assign_player_attributes(player: BasePlayer, is_dominant: bool, initial_capital: Currency):
    ...
    player.max_production = config.dominant_max_production if is_dominant else config.non_dominant_max_production
...
def _print_player_info(player: BasePlayer) -> None:
    """列印玩家資訊"""
    print(
        f"玩家 {player.id_in_group}: {'Dominant' if player.is_dominant else 'Non-dominant'}, "
...
def calculate_general_payoff(
    group: BaseGroup, tax_rate: float = 0, use_tax: bool = False, use_trading: bool = False
) -> None:
    """
...
def calculate_final_payoff_info(
    player: BasePlayer, cost_calculator_func: callable | None = None, additional_info_func: callable | None = None
) -> dict[str, Any] | None:
    """
    計算最終報酬資訊
...
def _calculate_cost_for_round(player: BasePlayer, cost_calculator_func: callable | None) -> float:
    """計算指定回合的成本"""
...
def get_production_template_vars(
    player: BasePlayer, treatment: str, additional_vars: dict[str, Any] | None = None
) -> dict[str, Any]:
    """
    獲取生產決策頁面的模板變數
