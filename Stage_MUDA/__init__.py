...
def _initialize_player(player: BasePlayer) -> None:
    ...
    print(f"玩家 {player.id_in_group} 的碳權價值: {player.personal_item_value} (持有數量: {player.current_items})")
...
class TradingMarket(Page):
    ...
    def live_method(player: Player, data: dict[str, Any]) -> dict[int, dict[str, Any]]:
        ...
        if data.get("type") == "submit_offer":
            ...
            result = process_new_order(player, group, direction, price, quantity, C.ITEM_NAME, "current_items")
        ...
        # 修復：統一處理 accept_offer，統一使用 player_id 參數
            ...
            result = process_accept_offer(
                player, group, offer_type, target_id, price, quantity, C.ITEM_NAME, "current_items"
            )
...
def _calculate_final_payoff_info(player: Player) -> dict[str, Any]:
    ...
    # 計算選中回合的資料
    personal_value = (
        selected_round_player.field_maybe_none("personal_item_value")
        or selected_round_player.subsession.item_market_price
    )
