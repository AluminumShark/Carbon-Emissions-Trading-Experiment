...
class TradingMarket(Page):
    ...
    def live_method(player, data):
        ...
        if data.get('type') == 'submit_offer':
            ...
            if d == 'sell' and player.current_items < qt:
                print(f"賣單失敗：物品不足，需要 {qt}，但只有 {player.current_items}")
                return {player.id_in_group: {'type': 'fail', 'message': '物品不足'}}
                ...
                if sell and len(sell) > 0:
                    ...
                    if float(lowest_sell[1]) <= pr:
                        ...
                        try:
                            ...
                            player.current_items += trade_qty
                            seller.current_items -= trade_qty
                            
                            # 更新統計
                            player.total_bought += trade_qty
                            player.total_spent += actual_price * trade_qty
                            seller.total_sold += trade_qty
                            seller.total_earned += actual_price * trade_qty
                            ...
                            for p in group.get_players():
                                ...
                                if p.id_in_group == player.id_in_group:
                                    state['notification'] = {
                                        'type': 'success',
                                        'message': f'交易成功：您以價格 {actual_price} 買入了 {trade_qty} 個{C.ITEM_NAME}'
                                    }
                                elif p.id_in_group == seller_id:
                                    state['notification'] = {
                                        'type': 'success',
                                        'message': f'交易成功：您以價格 {actual_price} 賣出了 {trade_qty} 個{C.ITEM_NAME}'
                                    }
                ...
                if buy and len(buy) > 0:
                    ...
                    if float(highest_buy[1]) >= pr:
                        ...
                        try:
                            ...
                            player.current_cash += actual_price * trade_qty
                            buyer.current_items += trade_qty
                            player.current_items -= trade_qty
                            
                            # 更新統計
                            buyer.total_bought += trade_qty
                            buyer.total_spent += actual_price * trade_qty
                            player.total_sold += trade_qty
                            player.total_earned += actual_price * trade_qty
            ...
            if ot == 'sell' and tid != player.id_in_group:
                    ...
                    try:
                        ...
                        for p in group.get_players():
                            ...
                            if p.id_in_group == player.id_in_group:
                                state['notification'] = {
                                    'type': 'success',
                                    'message': f'交易成功：您以價格 {pr} 買入了 {qt} 個{C.ITEM_NAME}'
                                }
                            elif p.id_in_group == tid:
                                state['notification'] = {
                                    'type': 'success',
                                    'message': f'交易成功：您以價格 {pr} 賣出了 {qt} 個{C.ITEM_NAME}'
                                }
                            market_states[p.id_in_group] = state
                ...
                if player.current_items >= qt:
                    ...
                    print(f"交易失敗：物品不足，需要 {qt}，但只有 {player.current_items}")
                    return {player.id_in_group: {'type': 'fail', 'message': '物品不足'}}
            else:
